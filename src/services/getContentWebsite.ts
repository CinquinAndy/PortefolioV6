...
export async function getAbout(locale: Locale): Promise<StrapiResponse<About> | NotFoundResponse> {
  ...
  const processedAbout = await processMarkdown(
    dataAbout.data.attributes.content
  )
 ...
 * Get content website
 */
export async function getContentWebsite(locale: Locale): Promise<StrapiResponse<ContentWebsite> | NotFoundResponse | undefined> {
  try {
    ...
    const processedContentCta = await processMarkdown(
      dataContentWebsite.data?.attributes?.cta?.content || ''
    )
...
export async function getNotFound(locale: Locale): Promise<StrapiResponse<NotFound> | NotFoundResponse> {
  const dataNotFound = await fetchAPI<StrapiResponse<NotFound>>(
  ...
  const processedNotFound = await processMarkdown(
    dataNotFound.data.attributes.content
  )
 ...
 */
export async function processArticleData(articleData: Article): Promise<Article & { data: { attributes: Article['attributes'] & { content: string } } }> {
  if (!articleData) {
    redirect('/404')
  }
...
export async function processRealisationData(
  ...
  if (!realisationData || !realisationData.data?.[0]) {
    redirect('/404')
  }
